...
void UMathProjectLibrary::Print_Log(ELogVerb LogVerb, FString Text, int Line, const char* Function)
{
    switch (LogVerb)
    {
        case ELogVerb::Display: UE_LOG(LogMath, Display, TEXT("[%s][%d] TEXT:[%s]"), ANSI_TO_TCHAR(Function), Line, *Text); break;

        case ELogVerb::Warning: UE_LOG(LogMath, Warning, TEXT("[%s][%d] TEXT:[%s]"), ANSI_TO_TCHAR(Function), Line, *Text); break;

        case ELogVerb::Error: UE_LOG(LogMath, Error, TEXT("[%s][%d] TEXT:[%s]"), ANSI_TO_TCHAR(Function), Line, *Text); break;
    }
}
